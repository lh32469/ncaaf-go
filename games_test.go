...
func TestGetOpponent(t *testing.T) {
	...
	opponent := getOpponent(michigan, 1, scoreBoards)
	...
	opponent = getOpponent(florida, 1, scoreBoards)
...
func TestGetOpponentNCState(t *testing.T) {
	...
	opponent := getOpponent(*ncState, 1, scoreBoards)
...
func TestGetOpponenOhioState(t *testing.T) {
	...
	opponent := getOpponent(*ohioState, 1, scoreBoards)
	log.Printf("opponent: %s\n", opponent)
	...
	scoreBoards = getScoreBoards(2018)

	opponent = getOpponent(*ohioState, 1, scoreBoards)
	log.Printf("opponent: %s\n", opponent)

	if "Oregon St." != opponent.Name {
		t.Logf("Opponent: %s\n", opponent)
		t.Fatalf(`Couldn't find Opponent for %s'`, ohioState)
	}
